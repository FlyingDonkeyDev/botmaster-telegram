...
// const express = require('express'); // No se usa con Telegraf
// const bodyParser = require('body-parser'); // No se usa con Telegraf
...
class TelegramBot extends BaseBot {
  ...
  constructor(settings) {
    ...
    this.telegraf = new Telegraf(this.credentials.authToken);
   ...
   */
  __createMountPoints() {
    // Usamos telegraf en su lugar
    this.telegraf.on('message', (ctx) => {
      this.__formatUpdate(ctx.update)
        .then((update) => {
  ...
  __formatOutgoingMessage(message) {
    ...
    if (message.message.attachment) {
      formattedMessage.attachment = message.message.attachment;
      /*
  ...
  __sendMessage(rawMessage) {
    return new Promise(async (resolve, reject) => {
      const extra = {};
      if (rawMessage.reply_markup) {
        extra.reply_markup = rawMessage.reply_markup;
      }

      if (rawMessage.action) {
        // Mandamos un typing
        resolve({
          result: await this.telegraf.telegram.sendChatAction(
            rawMessage.chat_id,
            rawMessage.action
          )
        });
        ...
        // Mandamos un texto
        resolve({
          result: await this.telegraf.telegram.sendMessage(
            rawMessage.chat_id,
            rawMessage.text,
            extra
          )
        });
      } else if (rawMessage.attachment) {
        // Mandamos una imagen
        resolve({
          result: await this.telegraf.telegram.sendPhoto(
            rawMessage.chat_id,
            rawMessage.attachment.payload,
            extra
          )
        });
      } else {
